...
// apps/electron-backend/src/app/workers/epg-streaming-parser.ts
var import_saxes = __toESM(require_saxes());
...
var StreamingEpgParser = class {
  constructor(onChannelsBatch, onProgramsBatch, onProgress, channelBatchSize = 100, programBatchSize = 1e3) {
    this.onChannelsBatch = onChannelsBatch;
    this.onProgramsBatch = onProgramsBatch;
    this.onProgress = onProgress;
    this.channelBatchSize = channelBatchSize;
    this.programBatchSize = programBatchSize;
    this.channels = [];
    this.programs = [];
    ...
    // For nested elements
    this.elementStack = [];
    this.parser = new import_saxes.SaxesParser();
    this.setupParser();
  }
  setupParser() {
      ...
      switch (tag.name) {
        ...
        case "programme":
          ...
          this.currentProgram = {
            ...
            channel: tag.attributes["channel"] || "",
            title: [],
            desc: [],
            category: [],
            date: "",
        ...
        case "icon":
          if (this.currentChannel) {
            this.currentChannel.icon.push({
              ...
              width: tag.attributes["width"] ? parseInt(tag.attributes["width"]) : void 0,
              height: tag.attributes["height"] ? parseInt(tag.attributes["height"]) : void 0
            });
        ...
        case "desc":
        case "category":
          this.currentLang = tag.attributes["lang"] || "";
          break;
        case "rating":
          if (this.currentProgram) {
            const system = tag.attributes["system"] || "";
      ...
      if (this.currentProgram) {
        switch (tag.name) {
          ...
          case "date":
            ...
            break;
          case "value":
            if (this.elementStack.includes("rating") && this.currentProgram.rating.length > 0) {
              this.currentProgram.rating[this.currentProgram.rating.length - 1].value = text;
            }
            break;
          case "episode-num":
            if (this.currentProgram.episodeNum.length > 0) {
              this.currentProgram.episodeNum[this.currentProgram.episodeNum.length - 1].value = text;
            }
            break;
          case "programme":
...
// apps/electron-backend/src/app/workers/epg-response-utils.ts
function hasGzipPath(url) {
  if (!url) {
    return false;
  }
  try {
...
function shouldGunzipEpgResponse(originalUrl, response) {
  if (hasGzipPath(originalUrl) || hasGzipPath(response.url)) {
    return true;
  }
  const contentType = response.headers.get("content-type");
  if (contentType && /(application\/gzip|application\/x-gzip)/i.test(contentType)) {
    return true;
  }
...
var CHANNEL_BATCH_SIZE = 100;
var PROGRAM_BATCH_SIZE = 1e3;
var EpgDatabase = class {
  constructor() {
   ...
   */
  insertChannels(channels, sourceUrl, clearFirst = false) {
    const insertMany = this.db.transaction((channels2) => {
      if (clearFirst) {
        this.deleteChannelsStmt.run(sourceUrl);
        this.knownChannelIds.clear();
      }
  ...
  insertPrograms(programs) {
    let insertedCount = 0;
    const insertMany = this.db.transaction((programs2) => {
      for (const prog of programs2) {
...
async function fetchAndParseEpgStreaming(url) {
  ...
  try {
    ...
    const parser = new StreamingEpgParser(
      ...
      (totalChannels, totalPrograms) => {
        ...
        import_worker_threads.parentPort?.postMessage(response2);
      },
      CHANNEL_BATCH_SIZE,
      PROGRAM_BATCH_SIZE
    );
      ...
      dataStream.on("end", () => {
        try {
          ...
          if (stats.totalChannels === 0) {
            ...
            import_worker_threads.parentPort?.postMessage(response3);
...
function clearAllEpgData() {
  ...
  try {
    ...
    const response = { type: "CLEAR_COMPLETE" };
    ...
    import_worker_threads.parentPort?.postMessage(errorResponse);
  } finally {
    db.close();
  }
}
if (import_worker_threads.parentPort) {
  import_worker_threads.parentPort.on("message", async (message) => {
    try {
      if (message.type === "FETCH_EPG" || message.type === "FORCE_FETCH") {
  ...
  import_worker_threads.parentPort.postMessage({ type: "READY" });
} else {
...
xmlchars/xmlns/1.0/ed3.js:
   ...
   *)
*/
//# sourceMappingURL=epg-parser.worker.js.map
